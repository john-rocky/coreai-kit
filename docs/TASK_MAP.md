...
| Raw transcript → written text | — (no Apple API to measure) | **nothing** — the transcriber returns what was said, fillers and false starts included; `NaturalLanguage` corrects spelling and does not remove them | S1-mini **796 MB** (iPhone-verified) | **Shipped 2026-08-25** as `CoreAI.tidyTranscript`. The piece every ASR row above was missing: a transcript is not yet text a person would send |
