...
```bash
python3 check.py http://127.0.0.1:8090              # 20 requests (cases.json) + the three routes around them
...
question types, the 16- and 17-option edges, 2 and 10 score levels, structured state and
instructions, a chat as the state, four malformed requests — and checks the status and the
shape of each answer (keys and types, never values). Extra keys are listed, not failed.
