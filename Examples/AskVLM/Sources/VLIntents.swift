...
struct VLAnswerQuery: EntityQuery {
    func entities(for identifiers: [String]) async throws -> [VLAnswerEntity] {
        // The id IS the answer text (see the query), so a re-resolved entity still carries the full
        // answer — the Visual Intelligence row truncates the title, and tapping it opens the app to
        // the FULL answer.
        identifiers.map { VLAnswerEntity(id: $0, answer: $0, thumbnail: nil) }
    }
}
...
struct AskVLMValueQuery: IntentValueQuery {
    ...
    func values(for input: SemanticContentDescriptor) async throws -> [VLAnswerEntity] {
        ...
        // id == the answer text, so a tap that re-resolves the entity still gets the full answer
        // (the Visual Intelligence row truncates the title; tapping opens the app to the full text).
        return [
            VLAnswerEntity(
                id: vlm.text, answer: vlm.text,
