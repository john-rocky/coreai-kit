// From john-rocky/coreai-model-zoo conversion/nemotron3_diar/swift/Sources/NemotronDiarizer/N3DMel.swift @ 26241ae (same author); the kit's copy is internal.
...
struct N3DMel: Sendable {
    ...
    static let logGuard: Float = 0x1p-24
    ...
    init(melFilters: [Float], hannWindow: [Float]) {
    ...
    static func validFrames(samples n: Int, center: Bool) -> Int {
        center ? floorDiv(n, hop) : floorDiv(n - nFFT, hop) + 1
    }

    /// Log-mel of one audio buffer: (mel [F, 128] frame-major, F).
    func logMel(_ audio: UnsafeBufferPointer<Float>, center: Bool) -> (mel: [Float], frames: Int) {
        let n = audio.count
    ...
    func logMel(_ audio: ArraySlice<Float>, center: Bool) -> (mel: [Float], frames: Int) {
    ...
    static func streamChunks(samples n: Int, chunkFrames c: Int, lookaheadFrames la: Int) -> [N3DChunk] {
...
struct N3DEmbedder: Sendable {
    ...
    func embed(mel: [Float], frames: Int) -> (embeds: [Float], rows: Int) {
