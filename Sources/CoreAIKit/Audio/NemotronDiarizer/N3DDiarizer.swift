// From john-rocky/coreai-model-zoo conversion/nemotron3_diar/swift/Sources/NemotronDiarizer/N3DDiarizer.swift @ 26241ae (same author); the kit's copy is internal.
...
struct N3DProfile: Sendable, Equatable {
    ...
    static func streamingProfile(mode: N3DStreamingMode) -> N3DProfile {
    ...
    static let offline = N3DProfile(kind: .offline, mode: nil, chunkFrames: 340, lookaheadFrames: 40,
...
struct N3DStepInput: Sendable {
    ...
    init(rows: [Float], lookahead: Int, emitFrames: Int) {
        self.rows = rows
        self.lookahead = lookahead
        self.emitFrames = emitFrames
    }

    var rowCount: Int { rows.count / N3DSpeakerCache.hidden }
}
...
struct N3DStepInfo: Sendable {
    ...
    let chunkFrames: Int
    let lookahead: Int
    let compressedAfter: Bool
    let cacheFramesAfter: Int
    let fifoFramesAfter: Int
...
struct N3DSegment: Sendable, Hashable {
    ...
    init(speaker: Int, startFrame: Int, endFrame: Int) {
        self.speaker = speaker
        self.startFrame = startFrame
        self.endFrame = endFrame
    }

    var start: Double { Double(startFrame) / 100 }
...
actor N3DDiarizer {
    ...
    nonisolated let profile: N3DProfile
    nonisolated let computeUnits: N3DComputeUnits
    ...
    let silence: [Float]
    private(set) var poison: N3DSpeakerCache.Poison?

    static func streamingProfile(mode: N3DStreamingMode) -> N3DProfile { .streamingProfile(mode: mode) }
    static var offline: N3DProfile { .offline }
    ...
    func setPoison(_ poison: N3DSpeakerCache.Poison?) { self.poison = poison }

    /// Diarize a whole 16 kHz mono recording. `profile` may switch between the streaming modes (same T).
    func process(samples: [Float], profile: N3DProfile? = nil,
    ...
    func process(steps: [N3DStepInput], frames: Int, profile: N3DProfile? = nil,
                        onStep: (@Sendable (N3DStepInfo) -> Void)? = nil) async throws -> N3DOutput {
    ...
    func runGraph(packed: [Float], valid: [Float]) async throws -> [Float] {
        try await graph.run(packed: packed, valid: valid)
    }

    nonisolated var graphLength: Int { profile.graphLength }
    ...
    static func segments(from probs: [Float], threshold: Float = 0.5) -> [N3DSegment] {
