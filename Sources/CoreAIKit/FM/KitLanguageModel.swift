...
public struct KitLanguageModel: LanguageModel {
    ...
    public init(
        ...
        try await self.init(bundleAt: url, engineVariant: engineVariant)
    }
    ...
    /// `engineVariant: .sequential` to enable guided generation.
    public init(bundleAt url: URL, engineVariant: EngineVariant = .auto) async throws {
